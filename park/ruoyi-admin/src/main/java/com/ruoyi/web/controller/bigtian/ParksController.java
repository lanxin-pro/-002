...
import com.ruoyi.bigtian.domain.Area;
import com.ruoyi.bigtian.domain.Bike;
...
import com.ruoyi.bigtian.service.AreaService;
import com.ruoyi.bigtian.service.IAttachmentService;
import com.ruoyi.bigtian.service.IParksService;
import com.ruoyi.bigtian.service.impl.BikeServiceImpl;
...
//        }
        ...
        Area area = new Area();
        area.setAid(1);
        area.setArea(parks.getName());
        Float v = new Float(0.003);
        area.setLeftlng(Float.parseFloat(parks.getLongitude()));
        float longitude = Float.parseFloat(parks.getLongitude());
        // 进行浮点数相加
        float newLongitude = longitude + v;
        // 赋值给 area 的 rightlng 属性
        area.setRightlng(newLongitude);

        area.setLeftlat(Float.parseFloat(parks.getLatitude()));

        float bbbbb = Float.parseFloat(parks.getLatitude());
        // 进行浮点数相加
        float newLongcccccitude = bbbbb + v;
        // 赋值给 area 的 rightlng 属性
        area.setRightlat(newLongcccccitude);

        area.setPrice(parks.getPrice().intValue());


        areaService.save(area);

        Bike bike = new Bike();
        bike.setAid(area.getId());
        bike.setNumber(parks.getName());
        bike.setLat(Float.parseFloat(parks.getLatitude()));
        bike.setLng(Float.parseFloat(parks.getLongitude()));
        bike.setDeposit(parks.getRates());
